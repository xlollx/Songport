...
| Amazon Music | experimental, via the [Songport Bridge](https://github.com/xlollx/Songport-YTM-Bridge) | Amazon sign-in inside the Bridge | nothing | Amazon's official Web API is a closed beta for approved partners, so the Bridge uses the web player's interface. Search and reading a playlist work; library listing and writing are still being mapped from captured traffic and fail with a clear message until then. |
